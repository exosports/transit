...
resample(const short getingx, /* Axis flag: 0:y, 1:x,  2:free arrays      */
         ...
         long noutx,          /* Number of points in resampled x axis     */
  ...
  case 1:
    ...
    ndat  = nrefx--;
    nout  = noutx--;
    x     = refx;     /* 'x' is assigned to the data x-axis */
    soutx = outx;

    /* If it is not first use of this function, free before allocating: */
    if(indx != NULL){
      free(indx);
      free(t);
    }
    ...
    for(j=0; j<nout; j++){
      ...
      /* In case the next output element is not bigger than the previous,
         then start looking from the beginning. \linelabel{nonascend} */
      if(val < refx[i])
        i = 0;
      else if(val == refx[f]){
        i = indx[j] = f++;

...
% http://planets.ucf.edu/bart-docs/transit_user_manual.pdf
...
emission flux spectrum of a hydrostatic atmosphere with
application to extrasolar-planet transit and eclipse observations,
respectively.  {\tt Transit} calculates the spectra by solving the
one-dimensional line-by-line radiative-transfer equation for an
 atmospheric model.  A separate document \findme{point
...
Section \ref{sec:installation} indicates how to obtain the code and the system
requirements.  
Section \ref{sec:quick-example} presents a Transit demonstration run
with minimal inputs.
Section \ref{sec:walkthrough} briefly describes a general picture of
the code, the required inputs, and the outputs.
Section \ref{sec:inputs} further details the inputs necessary
...
Section \ref{sec:organization} details the data structures.
...
# (*): TIPS code is implicit in Transit, set part_list to 'implicit'.
...
{ConfigParser}\footnote{\href{https://docs.python.org/2/library/configparser.html}
...
HITRAN   & H\sb{2}O, CO\sb{2}, CH\sb{4}, ...   & \href{http://cfa.harvard.edu/hitran/}{cfa.harvard.edu/hitran} \\
...
URLs for the databases listed in Table \ref{table:DBs}.  For the
HITRAN and HITEMP databases, {\pylineread} provides a modified version
of the Total Internal Partition Sums
...
to calculate the partition functions.  In this case, set the flag
{\tttm 'implicit'} in the {\tttm 'part\_list'} field.

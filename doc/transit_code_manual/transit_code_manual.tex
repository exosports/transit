...
The transit code is an object-oriented program. The main structure is
the {\tt transit struct}.
The user provided parameters (through the command-line arguments) are
stored in a {\tt transithint struct} before being handed to {\tt
  transit}.
The {\tt transithint} parameters can be modified or not accepted
(stopping the code) if they don't have an acceptable physical meaning.

Char arrays hold the input ({\tt f\_atm}, atmosphere file; {\tt
  f\_line}, TLI file), and output filenames ({\tt f\_out}, Modulation
spectrum; {\tt f\_toomuch}, toomuch; {\tt f\_outsample}, samples).
...
\item[-] Initialize \ttred{tr.ds.th} transithint.
\item[-] Set \ttred{th.fl, th.verbnoise, th.mass, th.tauiso} with
  default values (flags, verbose level, mass or number abundance bool,
  and per isotope optical depth calculation).
\item[-] Set \ttred{th.det.tau.name, th.det.ext.name, th.det.cia.name}
  (detailed output default names).
\item[-] Set \ttred{th.ncia, th.ciafile} from command line
  arguments (Number of CIA files and CIA filenames).
\item[-] Set \ttred{th.save.ext} from command line arguments
  (filename to save extinction array).
\item[-] Set rest of fields in \ttred{th.det.tau, th.det.ext, th.det.cia}
  from command line arguments.
\item[-] Set \ttred{th.minelow, th.solname} from command line
  arguments (low-energy minimum limit and light-ray path solution
  type).
\item[-] Set \ttred{th.f\_atm, th.f\_line, th.f\_out, th.f\_outsample,
    th.f\_toomuch} from command line arguments (atmosphere, line
  database, output, samplings, tau too much filenames).
\item[-] Set \ttred{th.ot} and update \ttred{th.fl} with {\tt
    TRU\_OUTTAU} from command line arguments (output optical depth
  instead of modulation bool).
\item[-] Set \ttred{th.allowrq} from command line arguments
  (lowest allowed cumulative abundance).
\item[-] Update \ttred{th.mass} from command line arguments.
\item[-] Fill \ttred{th.onept} fields.
\item[-] Update \ttred{th.fl} to include {\tt TRU\_ATMASK1P} flag
  if set by command line arguments.
\item[-] Set \ttred{th.rads.i, th.rads.f, th.rads.d, th.rads.fct} from
  command line arguments (radius sampling parameters).
\item[-] Set \ttred{th.wavs.i, th.wavs.f, th.wavs.d, th.wavs.fct,
    th.wavs.o} from command line arguments (wavelength sampling).
\item[-] Set \ttred{th.wavs.n, th.wavs.v} default values.
\item[-] Set \ttred{th.wns.i, th.wns.f, th.wns.d, th.wns.fct,
    th.wns.o} from command line arguments (wavenumber sampling).
\item[-] Set \ttred{th.wns.n, th.wns.v} default values.
\item[-] Set \ttred{th.margin, th.wnm} from command line arguments
  (wavelength and wavenumber margins).
\item[-] Set \ttred{th.maxratio\_doppler, th.voightfine,
    th.timesalpha} from command line arguments (Voigt's width to
  recalculate the profile, fine-binning oversampling, and half-width
  of profile in number of broadening half-widths).
\item[-] Update \ttred{th.verblevel} from command line arguments.
\item[-] Modify \ttred{th.fl} to include {\tt TRU\_EXTINPERISO}
  from command line arguments (calculate extinction per isotope flag).
\item[-] Set \ttred{th.blowex} from command line argument (extinction
  multiplicative factor).
\item[-] Set \ttred{th.sg} variables except \ttred{x, y, starmass,
    starmassfct, starradfct} from command line arguments.
\item[-] Set \ttred{th.toomuch, th.tauiso, th.taulevel, th.modlevel}
...
\paragraph{Variables Modified:}
\begin{enumerate}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Set \ttred{tr.f\_out} from {\tt th.f\_out} or default (main
  output filename).
\item[-] Copy \ttred{tr.f\_toomuch, tr.f\_outsample} from {\tt
    th.f\_toomuch, th.f\_outsample} (maximum optical depth, and
  sampling output filenames).
\item[-] Call to \ttblue{acceptsoltype} to set \ttred{tr.sol}
  (transit ray solution).
\item[-] Call to \ttblue{setgeomhint} to set orbital and stellar parameters.
  % \item[-] Copy \ttred{tr.ds.sg.transpplanet} from {\tt th.sg.transpplanet}.
  % \item[-] Set All \ttred{tr.ds.sg} variables except {\tt tr.ds.sg.x} and {\tt tr.ds.sg.y} from {\tt th.sg} or default values.
  % \item[-] Update \ttred{tr.pi} to include {\tt TRPI_GEOMETRYHINT}.
\item[-] Copy \ttred{tr.ds.det} from {\tt th.det}.
\end{enumerate}
...
This file is concerned with reading TLI files produced by the {\tt pylineread} program. Function \ttblue{savefile, main} are not currently used in {\tt transit}.
...
\function{
static void isisoline(&char *name, PREC\_ATM mass, int *isoeq, 
                   \\ &enum isodo atisodo, prop\_isof *isof, enum isodo *isodo,
                   \\ &PREC\_NREC nliso)}
Check whether the isotope with 'name' corresponds to an isotope in
lineinfofile. If yes, set's its isodo parameter. \newline

\function{
static int checknonmatch(&struct transit *tr, struct atm\_data *at,
                      \\ &enum isodo *isodo)}
Ask what to do with lineinfo isotopes that don't have a match in 
the atmosphere file. \newline

\function{
static inline double findfactq(&char *iso, prop\_isof *isof, prop\_isov *isov,
                            \\ &int n, PREC\_NREC r)}
Find the isotope in 'isof'  with name 'iso' and get its abundance at
radius given by index 'r'. \newline

\function{
static inline double addq(&prop\_isov *isov, enum isodo *isodo,
                       \\ &\_Bool *other, int n, PREC\_NREC r)}
...
\function{
int getmnfromfile(FILE *fp, struct atm\_data *at, struct transit *tr, int nmb)}
Get keyword variables from atmosphere file (mass/number abundance bool;
zero-radius offset; radius, temperature, and pressure units factor;
atmfile name/info; list isotopes; list of proportional-abundance isotopes).
Store molecules and proportional isotopes in atm\_data struct. 
...
\function{
int readatmfile(&FILE *fp, struct transit *tr, struct atm\_data *at,
             \\ &prop\_samp *rads, int nrad)}
...
\begin{enumerate}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Allocate \ttred{tr.ds.at.isoprop} (Atmosphere's
  proportional-isotopes array)
\item[-] Allocate \ttred{tr.ds.at.isodo, tr.ds.at.isoeq, tr.ds.at.m,
    tr.ds.at.n} (atmosphere's file isotope type, lineinfo's isotope
  index, isotope's mass, isotope's name).
\item[-] Set \ttred{tr.ds.at.mass} (abundance by mass (True) or number
  (False) boolean) from file.
\item[-] Set \ttred{tr.ds.at.rads.fct, tr.ds.at.atm.pfct,
    tr.ds.at.atm.tfct} (units factor of radius, pressure, and
  temperature arrays) from file.
\item[-] Call to {\tt storename} to set \ttred{tr.ds.at.info} from
  file.
\item[-] Re-allocate \ttred{tr.ds.at.isodo, tr.ds.at.isoeq,
    tr.ds.at.m, tr.ds.at.n} from file.
\item[-] Set \ttred{tr.ds.at.m, tr.ds.at.n} from file.
\item[-] Call to {\tt isisoline} to set \ttred{tr.ds.at.isoeq,
    tr.ds.at.isodo} from file.
\item[-] Re-allocate \ttred{tr.ds.at.isoprop} from file.
\item[-] Set \ttred{tr.ds.at.isoprop.m, tr.ds.at.isoprop.n} from file.
\item[-] Set \ttred{tr.ds.at.isoprop.f} (proportional factor) from
  file.
\item[-] Set \ttred{tr.ds.at.isoprop.t} (name of referenced isotope)
  from file.
\item[-] Call to {\tt isisoline} to set \ttred{tr.ds.at.isoprop.eq,
    tr.ds.at.isoprop.isodo} (lineinfo's isotope index, and isotope's
  type) from file.
\item[-] Set \ttred{tr.ds.at.begline} (position where tabulated
  data begins in atm file).
\item[-] Set \ttred{tr.ds.at.ipa, tr.ds.at.n\_aiso} (number of
  proportional isotopes, total number of isotopes in atmosphere file).
\item[-] Copy {\tt tr.ds.at.isoprop} isotopes into {\tt tr.ds.at}.
\item[-] Set \ttred{tr.ds.at.n\_niso} (number of non-ignored
  isotopes with no linelist).
\item[-] Add non-ignored non-linedb isotopes into {\tt tr.ds.at.iso}:
\item[-] Re-allocate 
  \ttred{tr.ds.iso.isodo, tr.ds.iso.isof, tr.ds.iso.isof.n}.
\item[-] Set \ttred{tr.ds.at.isoeq}.
\item[-] Copy \ttred{tr.ds.iso.isodo, tr.ds.iso.isof.m,
    tr.ds.iso.isof.n} from {\tt tr.ds.at.isodo, tr.ds.at.m,
    tr.ds.at.n}.
\item[-] Set \ttred{tr.ds.iso.n\_e} (number of non-ignored isotopes).
\end{enumerate}

\subsubsection{Variables Modified in readatmfile:}
\begin{enumerate}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Re-allocate \ttred{tr.ds.at.rads.v, tr.ds.at.atm.t,
    tr.ds.at.atm.p, tr.ds.at.mm,}\newline \ttred{tr.ds.iso.at.isov.d,
    tr.ds.at.isov.q} (atmosphere radius temperature, pressure,
  meanmolecular mass, isotopes' density, and isotopes' abundance
  arrays).
\item[-] Set \ttred{tr.ds.at.isov.n} (number of layers in the
  atmosphere).
\item[-] Set \ttred{tr.ds.at.rads.v, tr.ds.at.atm.p, tr.ds.at.atm.t}
  reading from file.
\item[-] Set \ttred{tr.ds.at.isov.q} interactively
  (tr.ds.at.isodo=fixed), from isoprop (factor), from file (atmfile),
  or call to {\tt addq} and calculate from isoprop (factor/other).
\item[-] Call to {\tt checkaddmm} to set \ttred{tr.ds.at.mm}.
\item[-] Call to {\tt stateeqnford} to set
  \ttred{tr.ds.at.isov.d}.
\end{enumerate}
...
int makesample(&prop\_samp *samp, prop\_samp *hint, prop\_samp *ref, const long fl,
            \\ &const float margini, const float marginf)}
...
Call makesample to create the wavelength sampling using
lineinfo.wavs as reference. \newline

\function{
int makewnsample(struct transit *tr)}
Call makesample to create the wavenumber sampling using the
inverse-wavelength values as reference. \newline

\function{
int makeradsample(struct transit *tr)}
Call makesample to create the radius sampling using tr.ds.at.rads
as reference.  
Interpolate to set tr.atm pressure, temperature, and mean molecular mass.  
Interpolate to set all isotopes density and abundance per radius.  
Interpolate to set linedb isotopes partition function and cross section. \newline

\function{
int makeipsample(struct transit *tr)}
Call makesample to create the impact parameter sampling using the
reversed radius limits and spacing as reference (produce always an
equispaced sampling). \newline
...
\subsubsection{makewnsample0:}
\paragraph{Variables Modified:}
\begin{itemize}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Create a reference wavenumber sample (\texttt{rsamp}).
\item[-] Set the initial \texttt{rsamp} wavenumber point from hinted
  wavenumber (or else from the inverted hited wavelength).
\item[-] Set the final \texttt{rsamp} wavenumber point from hinted
  wavenumber (or else from the inverted hited wavelength).
\item[-] Set the \texttt{rsamp} spacing from hinted wavenumber
  spacing.
\item[-] Call to \ttblue{makesample1} to set \ttred{tr.wns}.
\item[-] Updates the transit progress indicator (\ttred{tr.pi}) with
  \texttt{TRPI\_MAKEWN}. \newline
\end{itemize}
\paragraph{Walkthrough:}

\subsubsection{makewnsample0:}
Set wavenumber sampling variables and array.  Take values from hint
sampling.

\paragraph{Variables Modified:}
\begin{itemize}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Set units factor \ttred{tr.samp.fct}.
\item[-] Set initial value \ttred{tr.samp.i}.
\item[-] Set final value \ttred{tr.samp.f}.
\item[-] Check that there is a non-zero interval. 
\item[-] Set spacing \ttred{tr.samp.d}.
\item[-] Calculate numpoints (\ttred{tr.samp.n}), oversample
    (\ttred{tr.samp.o}), and fill in the array values
    (\ttred{tr.samp.v}).
\end{itemize}
\paragraph{Walkthrough:}


\subsubsection{makewavsample:}
\paragraph{Variables Modified:}
\begin{enumerate}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Call to makesample to set \ttred{tr.wavs} values.
\item[-] Modify \ttred{tr.pi} to account for {\tt TRPI\_MAKEWAV}.
\end{enumerate}
\paragraph{Walkthrough:}
...
\paragraph{Variables Modified:}
\begin{enumerate}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Set \ttred{tr.wnmf, tr.wnmi} from {\tt th.wnm}
  (wavenumber margins) if defined, else set the margins from the
  inverse wavelength margin values.
\item[-] Call to makesample to set \ttred{tr.wns} values.
...
\item[-] Allocate \ttred{tr.ds.iso.isov.d, tr.ds.iso.isov.q,
    tr.ds.iso.isov.c, tr.ds.iso.isov.z} (isotope's density, abundance,
  cross section, partition function).
...
\item[-] Check that \ttblue{getatm} and \ttblue{readinfo\_tli} have
  been executed.
\item[-] Set flag (\texttt{SAMP\_LINEAR} or \texttt{SAMP\_SPLINE}) to
  define linear or spline interpolation.
\item[-] Call to \ttblue{makesample0} to set \ttred{tr.rads} (uses
  atmosphere data radius sampling as reference when there are no
  hinted values).
\item[-] Call to \ttblue{resamplex} and \ttblue{resampley} to
  interpolate atmospheric's temperature (\ttred{tr.atm.t}), pressure
  (\ttred{tr.atm.p}), and mean molecular mass (\ttred{tr.atm.mm})
  profiles according to the equispaced radius sampling (\ttred{tr.rads.v}).
\item[-] Call to \ttblue{resamplex} and \ttblue{resampley} to
  interpolate the isotopes density (\ttred{tr.ds.iso.isov.d}) and
  abundance (\ttred{tr.ds.iso.isov.q}) profiles according to the
  radius sampling.
\item[-] Interpolate the isotopes' collisional cross-section
  (\ttred{tr.ds.iso.isov.c}) and partition-function
  (\ttred{tr.ds.iso.isov.z}) profiles according to the atmospheric
  thermal profile.
\item[-] Updates the transit progress indicator (\ttred{tr.pi}) with
  \texttt{TRPI\_MAKERAD}. \newline
\end{itemize}
...
\paragraph{Variables Modified:}
\begin{enumerate}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Call to makesample to set \ttred{tr.ips} values.
\item[-] Modify \ttred{tr.pi} to account for {\tt TRPI\_MAKEIP}.
\end{enumerate}

\paragraph{Walkthrough:}
\begin{itemize}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Create a mock impact parameter sample (\texttt{usamp}) from hint
  values.
\item[-] Create a mock impact parameter sample (\texttt{rsamp}) as
  reference from reversed radius sampling.
\item[-] Check that final value is smaller than the initial value in
  hint sample.
\item[-] Check that \ttblue{makeradsample} has been called.
\item[-] Call to \ttblue{makesample0} to set \ttred{tr.ips} (uses
  rsamp sampling as reference when there are no hinted values).
\item[-] Updates the transit progress indicator (\ttred{tr.pi}) with
  \texttt{TRPI\_MAKEIP}.
\end{itemize}
...
\paragraph{Variables Modified:}
\begin{enumerate}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Set \ttred{tr.samp.fct} from hint sample, else from
  reference sample (sample units factor).
\item[-] Set \ttred{tr.samp.i, tr.samp.f} from hint sample or from
  reference sample (sample initial and final values).
\item[-] Set \ttred{tr.samp.n, tr.samp.v} from hint, else from
  reference sample if \ttred{n} given. Set \ttred{tr.samp.d}
  to 0 or $-1$.
\item[-] Set \ttred{tr.samp.d} from hint, else from reference
  sample if d given. Calculate \ttred{tr.samp.n, tr.samp.v} based
  on {\tt tr.samp.d}. Set \ttred{tr.samp.o} from hint, else from
  reference sample.
\end{enumerate}


\subsubsection{makesample0:}
Set (radius or impact parameter) sampling variables and array.  Take
values from hint or else from the reference sampling. \newline

\paragraph{Variables Modified:}
\begin{itemize}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
\item[-] Set units factor (\ttred{tr.samp.fct}).
\item[-] Set initial value \ttred{tr.samp.i} from hint (or else from
  reference).
\item[-] Set final value \ttred{tr.samp.f} from hint (or else from
  reference).
\item[-] If no sampling was provided and reference numpoints was
  provided, assume an array of values exists, set \ttred{tr.samp.n}
  and \ttred{tr.samp.v}, set oversampling (\ttred{tr.samp.o}), and
  return.
\item[-] If spacing was provided in hint or reference:
  \begin{itemize}[leftmargin=10pt, noitemsep, parsep=0pt, topsep=0ex]
  \item[-] Set \ttred{tr.samp.d}.
  \item[-] Check that there is a non-zero interval. 
  \item[-] Set \ttred{tr.samp.o} from hint or reference.
  \item[-] Calculate numpoints (\ttred{tr.samp.n}), oversample
    (\ttred{tr.samp.o}), and fill in the array values
    (\ttred{tr.samp.v}). \newline
  \end{itemize}
...
\subsubsection{outsample and printsample:}
\paragraph{Variables Modified:}
  - Set pointer to file \ttred{tr.f\_outsample}. \\
  - Call to {\tt printsample} to print each sampling: \\
   - Print the sample's \ttred{fct, i, f, d, o, n} parameters. \\
   - Print the sample's \ttred{v} array.

\paragraph{Walkthrough:}
outsample opens the file tr.f\_outsample to write the wavelength,
wavenumber, radius, and impact sampling parameters. It calls to
printsample to print each one.
...
\item[-] Modify \ttred{tr.pi} to account for {\tt TRPI\_IDXREFRAC}.
\end{enumerate}

\paragraph{Walkthrough}
Should calculate the index of refraction at each radius in the
atmosphere. Currently, it sets an index of refraction of 1.0 at all
levels (no light bending).

\newpage
\subsection{extinction.c: EDITED}
This file contains routines associated with computing molecular extinction.
...
\item[-] Allocate \ttred{ex.e}.
\item[-] Allocate \ttred{ex.computed}.

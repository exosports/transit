...
makeradsample(struct transit *tr){
  ...
  for(i=0; i<nmol; i++){
    molec[i].d = splinterp(rsamp->n, rsamp->v, atms->molec[i].d, nrad, rad->v, molec[i].d);
    molec[i].q = splinterp(rsamp->n, rsamp->v, atms->molec[i].q, nrad, rad->v, molec[i].q);
  }

  for(i=0;i<nrad;i++)
    transitprint(1,30,"molec[%i].q[%i] is: %f \n",0, i, molec[0].q[i]);
  ...
  for(i=0; i<ndb; i++){       /* For each database separately:              */
    ...
    for(j=0; j < iso->db[i].i; j++){
      transitASSERT(iso1db + j > niso-1,
                    ...
                    "limit (%i).\n", iso1db+j, niso-1);
      iso->isov[iso1db+j].z = splinterp(li->db[i].t, li->db[i].T, isovs[j].z, nrad, atmt->t, iso->isov[iso1db+j].z);
    }
  }
